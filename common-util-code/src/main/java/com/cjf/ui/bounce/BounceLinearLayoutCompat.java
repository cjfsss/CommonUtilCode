...
public class BounceLinearLayoutCompat extends LinearLayoutCompat implements BounceAction {
    ...
    public BounceLinearLayoutCompat(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.BounceLinearLayoutCompat, defStyleAttr, 0);
        try {
            int orientation = array.getInt(R.styleable.BounceLinearLayoutCompat_BounceOrientation, LinearLayout.HORIZONTAL);
            float resistance = array.getFloat(R.styleable.BounceLinearLayoutCompat_BounceResistance, 3f);
            int duration = array.getInteger(R.styleable.BounceLinearLayoutCompat_BounceDuration, 300);
            if (resistance < 1) {
                resistance = 1f;
            }
