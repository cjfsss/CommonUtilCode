...
import com.cjf.util.extension.CommonExtKt;
 ...
 */
public class XTextInputEditText extends RegexTextInputEditText  {
    ...
    private List<OnFocusChangeListener> mXFocusChangeListeners;
    private String mOldTrimmedStr;

    public XTextInputEditText(Context context) {
        this(context, null);
    }

    public XTextInputEditText(Context context, AttributeSet attrs) {
        this(context, attrs, com.google.android.material.R.attr.editTextStyle); // Attention here !
    }

    public XTextInputEditText(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        setOnFocusChangeListener(new OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                ...
                Log.i("XEditText", "onFocusChange: " + mOldTrimmedStr + "newString: " + getTrimmedString());
                if (mXFocusChangeListeners == null || mXFocusChangeListeners.isEmpty() ||
                        TextUtils.isEmpty(getTrimmedString()) ||
                        TextUtils.equals(mOldTrimmedStr, getTrimmedString())) {
                    return;
                }
                mOldTrimmedStr = getTrimmedString();
                for (OnFocusChangeListener listener : mXFocusChangeListeners) {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.XTextInputEditText, defStyleAttr, 0);

        mSeparator = a.getString(R.styleable.XTextInputEditText_x_separator);
        if (mSeparator == null) {
            mSeparator = "";
        }
        ...
        disableClear = a.getBoolean(R.styleable.XTextInputEditText_x_disableClear, false);
        boolean togglePwdDrawableEnable = a.getBoolean(R.styleable.XTextInputEditText_x_togglePwdDrawableEnable, false);

        if (!disableClear) {
            int cdId = a.getResourceId(R.styleable.XTextInputEditText_x_clearDrawable, -1);
            if (cdId == -1)
        ...
        if (togglePwdDrawableEnable || (inputType == 129 || inputType == 145 || inputType == 18 || inputType == 225)) {
            ...
            mShowPwdResId = a.getResourceId(R.styleable.XTextInputEditText_x_showPwdDrawable, -1);
            mHidePwdResId = a.getResourceId(R.styleable.XTextInputEditText_x_hidePwdDrawable, -1);
            if (mShowPwdResId == -1)
            ...
            if (mShowPwdResId == R.mipmap.x_et_svg_ic_show_password_24dp ||
                    mHidePwdResId == R.mipmap.x_et_svg_ic_hide_password_24dp) {
            ...
            if (!disableClear) {
                mBitmap = getBitmapFromVectorDrawable(context, cdId,
                        cdId == R.mipmap.x_et_svg_ic_clear_24dp); // clearDrawable
            }
        }

        disableEmoji = a.getBoolean(R.styleable.XTextInputEditText_x_disableEmoji, false);

        String pattern = a.getString(R.styleable.XTextInputEditText_x_pattern);
        if (!mSeparator.isEmpty() && !isPwdInputType && pattern != null && !pattern.isEmpty()) {
            ...
            if (!ok) {
                Log.e("XTextInputEditText", "the Pattern format is incorrect!");
            }
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (hasFocused && mBitmap != null && isPwdInputType && !isTextEmpty()) {
            int left = getMeasuredWidth() - getPaddingRight() -
                    mTogglePwdDrawable.getIntrinsicWidth() - mBitmap.getWidth() - CommonExtKt.dp2px(this,4);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (hasFocused && isPwdInputType && event.getAction() == MotionEvent.ACTION_UP) {
            ...
            if (!disableClear) {
                right -= w + CommonExtKt.dp2px(this,4);
    ...
    private class MyTextWatcher implements TextWatcher {
        ...
        public void beforeTextChanged(CharSequence s, int start, int count, int after) {
            mOldLength = s.length();
            // 记录之前的文字
            mOldTrimmedStr = String.valueOf(s);
    ...
    public void addFocusChangeListener(OnFocusChangeListener listener) {
        if (mXFocusChangeListeners == null) {
            mXFocusChangeListeners = new ArrayList<>();
        }
        if (!mXFocusChangeListeners.contains(listener)) {
            mXFocusChangeListeners.add(listener);
        }
    }
    ...
    public void removeFocusChangeListener(OnFocusChangeListener listener) {
        ...
        if (mXFocusChangeListeners.contains(listener)) {
            mXFocusChangeListeners.remove(listener);
        }
    }
    ...
    public void setOnSearchListener(OnSearchListener listener) {
        setOnEditorActionListener((v, actionId, event) -> {
            if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                if (listener != null) {
                    listener.onSearch(getTrimmedString());
                }
                hideSoftInput(v);
            }
            return false;
